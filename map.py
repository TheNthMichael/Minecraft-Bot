...
def can_move_preconditions(y_level, ground, air1, air2, air3):
    ...
    air3_predicate = True
    if air3 is not None:
        air3_predicate = air3.type == "air" or air3.type == "uncertain"
    air3_predicate = (y_level == 0 or air3_predicate)

    air2_predicate = True
    if air2 is not None:
        air2_predicate = air2.type == "air" or air2.type == "uncertain"

    air1_predicate = True
    if air1 is not None:
        air1_predicate = air1.type == "air" or air1.type == "uncertain"

    ground_predicate = True
    if ground is not None:
        ground_predicate = ground.type != "air" or ground.type == "uncertain"
...
class QueryMap:
    def __init__(self, share_lock=None) -> None:
        """
    ...
    def update(self, element: MapNode, only_if_uncertain=True) -> None:
        ...
        if old_element is None:
            self.map[element.position] = element
        elif (not only_if_uncertain) or old_element.type == "uncertain":
